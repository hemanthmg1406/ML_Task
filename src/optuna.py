...
def objective(trial):
    ...
    # Use the Top 100 list from your config
    ...
    # 3. Updated Hyperparameter Space (Shallow & Wide)
    params = {
        'n_estimators': trial.suggest_int('n_estimators', 1000, 3000),
        'max_depth': trial.suggest_int('max_depth', 3, 5),          # Shallower trees for more features
        'learning_rate': trial.suggest_float('learning_rate', 0.005, 0.03, log=True),
        'min_child_weight': trial.suggest_int('min_child_weight', 50, 200), # Higher floor for stability
        'subsample': trial.suggest_float('subsample', 0.5, 0.7),
        'colsample_bytree': trial.suggest_float('colsample_bytree', 0.3, 0.5), # Force feature diversity
        'reg_alpha': trial.suggest_float('reg_alpha', 0.5, 20.0, log=True),
        'reg_lambda': trial.suggest_float('reg_lambda', 20.0, 150.0, log=True), # Aggressive L2
        'random_state': config.RANDOM_STATE,
        'n_jobs': -1
    }

    # 4. Train on Transformed Target
    model = xgb.XGBRegressor(**params)
    model.fit(X_train_qt[feats], y_train_t)
    
    # 5. Adversarial Scoring (Back to Real Units)
    X_val = X_test_qt[feats]
    X_std = X_val.std(axis=0).values
    
    # Helper to predict and invert
    def predict_real(data):
        return target_proc.inverse_transform(model.predict(data))

    # A. Baseline R² (Real Units)
    base_preds = predict_real(X_val)
    base_r2 = r2_score(y_test, base_preds)
    
    # B. Noise R² (1% relative noise)
    noise_data = X_val + 0.01 * np.random.normal(0, 1, X_val.shape) * X_std
    noise_r2 = r2_score(y_test, predict_real(noise_data))
    
    # C. Drift R² (0.5σ shift)
    drift_data = X_val + 0.5 * X_std
    drift_r2 = r2_score(y_test, predict_real(drift_data))

    # 6. Scoring with high penalty for drift failure
    noise_drop = max(0, base_r2 - noise_r2)
    drift_drop = max(0, base_r2 - drift_r2)
    
    # Final Score: Prioritize a high Drifted R2
    # If drift_r2 is negative, the score will collapse, guiding Optuna away
...
if __name__ == "__main__":
    ...
    study.optimize(objective, n_trials=100, n_jobs=1) # Run trials sequentially but use -1 in XGB
